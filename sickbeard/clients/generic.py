...
class GenericClient(object):
    ...
    def _request(self, method='get', params={}, data=None, files=None):
        ...
        logger.log(self.name + u': Requested a ' + method.upper() + ' connection to url '+ self.url + ' with Params= ' + str(params) + ((' Data=' + str(data)[0:100] + ('...' if len(data) > 100 else '')) if data is not None else ""), logger.DEBUG)
    ...
    def sendTORRENT(self, result):
        ...
        try:
            if result.url.startswith('magnet') or self.name == 'DownloadStation':
